...
class GenerateDataByDispatch implements SelfHandling, ShouldQueue
{
    ...
    public function __construct ($date = '')
    {
        ...
        else $this->date = $this->carbon->yesterday();
    ...
    public function handle()
    {
		...
		foreach($this->getAffiliateRevenueTracker() as $revenueTracker) {
			...
			if(!$revenueTracker) continue;
            // Discontinue if no records for survey takers and clicks
			if(!$revenueTracker->clicksVsRegistrationStatistics) continue;
			if($revenueTracker->clicksVsRegistrationStatistics['registration_count'] <= 0) continue;
			if($revenueTracker->clicksVsRegistrationStatistics['clicks'] <= 0) continue;
