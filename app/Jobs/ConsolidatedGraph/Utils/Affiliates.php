...
class Affiliates extends \App\Http\Services\Consolidated\Utils\Affiliates
{
    ...
    {
        if(array_key_exists(1, $benchmarks)) $this->campaignIDs[] = $benchmarks[1];
        if(array_key_exists(2, $benchmarks)) $this->campaignIDs[] = $benchmarks[2];
    }
    ...
    public function setOfferID ($exitPage)
    {
        ...
        if($exitPage instanceof \App\Campaign) $this->offerID = $exitPage->linkout_offer_id;
        if($exitPage instanceof \Illuminate\Database\Eloquent\Collection) {
            if(count($exitPage) > 0) $this->offerID = $exitPage[0]->linkout_offer_id;
        }
    }
    ...
    {
        if(!$this->affiliates->isEmpty()) return true;
        return false;
    }
}
