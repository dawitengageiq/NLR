...
trait AffiliateReport
{
    ...
    {
        if(!$affiliateReport instanceof \Illuminate\Database\Eloquent\Collection) return;
        ...
        foreach($this->affiliateReports as $report){
            if(in_array($report['campaign_id'], $campaigns[1])) $coreg1Revenue += $report['revenue'];
            if(in_array($report['campaign_id'], $campaigns[2])) $coreg2Revenue += $report['revenue'];
            if(in_array($report['campaign_id'], $campaigns[8])) $coreg3Revenue += $report['revenue'];
            if(in_array($report['campaign_id'], $campaigns[13])) $coreg4Revenue += $report['revenue'];
        }
        ...
        if(!array_key_exists('lsp_revenue', $this->revenues)) $this->revenues['lsp_revenue'] = 0;
