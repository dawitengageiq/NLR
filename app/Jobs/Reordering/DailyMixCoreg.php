...
class DailyMixCoreg
{
     ...
     * @param  Helpers\Leads                     $leads
     * @param  Helpers\RevenueTrackers           $revenueTrackers
     * @param  Helpers\MixedCoregCampaignOrder   $campaignOrder
     * @param  Helpers\CampaignViewReports       $campaignViewReports
     * @param  Utils\Calculate                   $calculate
     * @param  Utils\Ordering                    $ordering
     */
        ...
        )
    {
        ...
        if(!$this->debug) $this->revenueTrackers->setTime($now->hour);
    }
    ...
    public function execute ()
    {
        ...
        if($this->settings->getValue('mixed_coreg_campaign_reordering_status') != 1) return;
        ...
        // Stop if no records
        if($this->revenueTrackers->notExists()) return;
        ...
        foreach($this->revenueTrackers->getRevenueTracker() as $revenueTracker) {
            ...
            if(!$this->debug) $this->leads->setReferenceDAte($this->revenueTrackers->referenceDate());
            // Then query
            $this->leads->query();
            // Skip this revenue tracker if no leads where fetch.
            if($this->leads->notExists()) continue;
    ...
    public function skipThis ()
    {
        ...
        if($this->revenueTracker->mixed_coreg_campaign_views == 0) $this->revenueTracker->mixed_coreg_campaign_views = $this->settings->getValue('campaign_type_view_count');

        foreach($this->revenueTracker->campaignViewReports as $campaignViewReport)
        {
            if(!is_object( $campaignViewReport->campaignStatusInfo)) return true;

            if($campaignViewReport->campaignStatusInfo->status == 1
            && $campaignViewReport->affiliate_campaign_record->count == 0) return true;
            ...
            || $campaignViewReport->current_view_count < $this->revenueTracker->mixed_coreg_campaign_views) return true;
        }
