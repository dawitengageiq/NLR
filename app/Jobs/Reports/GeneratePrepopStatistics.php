...
class GeneratePrepopStatistics extends Job implements SelfHandling, ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach($revenueTrackers as $revenueTracker)
        {
            ...
            $noPrepopPercentage = (doubleval($noPrepopCount) / doubleval($clicksCount));
            $prepopWithErrorsPercentage = (doubleval($prepopWithErrorsCount) / doubleval($clicksCount));
            ...
            $profitMargin = 0.0;
            $numerator = doubleval($affiliateReport->revenue) - doubleval($affiliateReport->payout);

            if($numerator>0.0)
            {
                //$profitMargin = ($numerator / doubleval($affiliateReport->revenue)) * 100.00;
                $profitMargin = ($numerator / doubleval($affiliateReport->revenue));
            }
