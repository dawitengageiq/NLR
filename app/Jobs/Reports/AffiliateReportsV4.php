...
class AffiliateReportsV4 extends Job implements SelfHandling, ShouldQueue
{
    ...
    public function deleteCurrentRecord($affiliate_id = null, $rev_tracker = null, $campaign_id = null) {
        ...
        if($rev_tracker != null) $affiliate_reports->where('revenue_tracker_id', $rev_tracker);
        if($affiliate_id != null) $affiliate_reports->where('affiliate_id', $affiliate_id);
        if($campaign_id != null) $affiliate_reports->where('campaign_id', $campaign_id);
        $affiliate_reports->delete(); 

        $cake_revenues = DB::table('cake_revenues')->where('created_at', $date);
        if($rev_tracker != null) $cake_revenues->where('revenue_tracker_id', $rev_tracker);
        if($affiliate_id != null) $cake_revenues->where('affiliate_id', $affiliate_id);
        ...
        if($campaign_id == null) {
            ...
            if($rev_tracker != null) $revenue_tracker_cake_statistics->where('revenue_tracker_id', $rev_tracker);
            if($affiliate_id != null) $revenue_tracker_cake_statistics->where('affiliate_id', $affiliate_id);
            $revenue_tracker_cake_statistics->delete();

            $prepop_statistics = DB::table('prepop_statistics')->where('created_at', $date);
            if($rev_tracker != null) $prepop_statistics->where('revenue_tracker_id', $rev_tracker);
            if($affiliate_id != null) $prepop_statistics->where('affiliate_id', $affiliate_id);
            $prepop_statistics->delete();
        }

        $click_logs = DB::table('click_log_trace_infos')->whereBetween('click_date', [$date.' 00:00:00', $date.' 23:59:59']);
        if($rev_tracker != null) $click_logs->where('revenue_tracker_id', $rev_tracker);
        if($affiliate_id != null) $click_logs->where('affiliate_id', $affiliate_id);
        $click_logs->delete(); 
                    ...
                    foreach($s3d as $s4 => $s4d) {
                        foreach($s4d as $s5 => $data) {
                            ...
                            if($data['uclick'] > 0) {
                                ...
                                //PrePop Statistics
                                $npp = $data['no_prepop_count'] == 0 && $data['uclick'] == 0 ? 0 : (doubleval($data['no_prepop_count']) / doubleval($data['uclick']));
                                $pwep = $data['prepop_werrors_count'] == 0 && $data['uclick'] == 0 ? 0 : (doubleval($data['prepop_werrors_count']) / doubleval($data['uclick']));
    ...
    public function getRevTrackerLeadRevenue($affiliate_id = null, $rev_tracker = null) {
        ...
        if($affiliate_id != null) $leads->where('affiliate_revenue_trackers.affiliate_id', $affiliate_id);
        if($rev_tracker != null) $leads->where('leads.affiliate_id', $rev_tracker);
        ...
        while(true) {
            ...
            if(isset($stats['response']['data'])) {
                if(count($stats['response']['data']['data']) == 0) {
                    ...
                    if($stats['response']['data']['pageCount'] == $page) break;
                }
            }
        ...
        if($response->summary->no_of_records > 0) {
            foreach($response->placements->placement as $placement) {
                foreach($placement->transactions->transaction as $transaction) {
                    ...
                    $data[$revenue_tracker_id][$s1][$s2][$s3][$s4][$s5] += (double)$transaction->revenue;
                }
            }
            ...
            if(isset($session_response['result']['sessionUUID'])) {
                ...
                if(isset($xml->result->report->groups->group->groupRecords->record)) {
                    foreach($xml->result->report->groups->group->groupRecords->record as $value) {
                        ...
                        $revenue = doubleval($value->publisherRevenueShare);
    ...
    public function runAffiliateRestrict() {
        ...
        if($this->revTrackerRestrict == 1) {
            ...
            }else if($this->affiliateRestrict != '') {
