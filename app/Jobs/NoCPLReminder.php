...
class NoCPLReminder extends Job implements SelfHandling, ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $campaign_ids = [];
        foreach($campaigns as $c) $campaign_ids[] = $c->id;

        //Check if Existing Campaigns has CPL already or excluded campaign
        foreach($existing_campaigns as $cid) {
            if(!in_array($cid, $campaign_ids)) $updated_campaigns[] = $cid;

            if(in_array($cid, $excluded_campaigns)) $updated_campaigns[] = $cid;
        }
