    ...
    public function handle()
    {
        ...
        foreach($settings as $set) {
            switch($set->code) {
                ...
                case 'report_recipient':
                    ...
                    if(count($report_recipients) == 0) $report_recipients = ['karla@engageiq.com'];
                ...
                case 'qa_recipient':
                    ...
                    if(count($qa_recipients) == 0) $qa_recipients = ['karla@engageiq.com'];
                    break;
        ...
        **/
        ...
        foreach($leads as $lead)
        {
            ...
            /*FOR CHECKING*/ if(! isset($result[$lead->campaign_id][$lead->affiliate_id])) $result[$lead->campaign_id][$lead->affiliate_id] = [];

            //get total count
            if(! array_key_exists('total_count',$report[$lead->campaign_id][$lead->affiliate_id])) $report[$lead->campaign_id][$lead->affiliate_id]['total_count'] = 0;
            $report[$lead->campaign_id][$lead->affiliate_id]['total_count'] += 1;

            //for campaigns total count
            if(!array_key_exists('total_count',$campaign_reports[$lead->campaign_id])) $campaign_reports[$lead->campaign_id]['total_count'] = 0;
            ...
            if($lead->lead_status == 2 || $lead->lead_status == 0)
            {
                ...
                if(! array_key_exists('reject_count',$report[$lead->campaign_id][$lead->affiliate_id])) $report[$lead->campaign_id][$lead->affiliate_id]['reject_count'] = 0;     
                $report[$lead->campaign_id][$lead->affiliate_id]['reject_count'] += 1;

                //for campaign reports - get reject count & rejected percentage
                if(! array_key_exists('reject_count',$campaign_reports[$lead->campaign_id])) $campaign_reports[$lead->campaign_id]['reject_count'] = 0;     
                $campaign_reports[$lead->campaign_id]['reject_count'] += 1;

                if($this->strposa($lead_status, $accepted_keywords)) { //Acceptable Rejects
                    if(! array_key_exists('acceptable_count',$report[$lead->campaign_id][$lead->affiliate_id])) $report[$lead->campaign_id][$lead->affiliate_id]['acceptable_count'] = 0;
                    $report[$lead->campaign_id][$lead->affiliate_id]['acceptable_count'] += 1;

                    if(! array_key_exists('acceptable_count',$campaign_reports[$lead->campaign_id])) $campaign_reports[$lead->campaign_id]['acceptable_count'] = 0;
                ...
                }else if($this->strposa($lead_status, $duplicate_keywords)) { //Duplicate Rejects
                    if(! array_key_exists('duplicate_count',$report[$lead->campaign_id][$lead->affiliate_id])) $report[$lead->campaign_id][$lead->affiliate_id]['duplicate_count'] = 0;
                    $report[$lead->campaign_id][$lead->affiliate_id]['duplicate_count'] += 1;

                    if(! array_key_exists('duplicate_count',$campaign_reports[$lead->campaign_id])) $campaign_reports[$lead->campaign_id]['duplicate_count'] = 0;
                ...
                }else if($this->strposa($lead_status, $prepop_keywords)) { //Pre-pop Rejects
                    if(! array_key_exists('prepop_count',$report[$lead->campaign_id][$lead->affiliate_id])) $report[$lead->campaign_id][$lead->affiliate_id]['prepop_count'] = 0;
                    $report[$lead->campaign_id][$lead->affiliate_id]['prepop_count'] += 1;

                    if(! array_key_exists('prepop_count',$campaign_reports[$lead->campaign_id])) $campaign_reports[$lead->campaign_id]['prepop_count'] = 0;
                ...
                }else if($this->strposa($lead_status, $filter_keywords)) { //Filter Rejects
                    if(! array_key_exists('filter_count',$report[$lead->campaign_id][$lead->affiliate_id])) $report[$lead->campaign_id][$lead->affiliate_id]['filter_count'] = 0;
                    $report[$lead->campaign_id][$lead->affiliate_id]['filter_count'] += 1;

                    if(! array_key_exists('filter_count',$campaign_reports[$lead->campaign_id])) $campaign_reports[$lead->campaign_id]['filter_count'] = 0;
                    ...
                    if(! array_key_exists('other_count',$report[$lead->campaign_id][$lead->affiliate_id])) $report[$lead->campaign_id][$lead->affiliate_id]['other_count'] = 0;
                    $report[$lead->campaign_id][$lead->affiliate_id]['other_count'] += 1;

                    if(! array_key_exists('other_count',$campaign_reports[$lead->campaign_id])) $campaign_reports[$lead->campaign_id]['other_count'] = 0;
        ...
        if(count($report) > 0) {
            ...
            foreach($sort_total as $fid => $total_count) {
                ...
                if(isset($report[$campaign_id][$revenue_tracker_id]['duplicate_count'])) {
                    ...
                    if($duplicate_ratio == 1) $only_duplicates = true;
                    ...
                    foreach($rej_campaigns as $rejc) {
                        ...
                        if($full_rejection_advertiser_email_status == 1) {
                            ...
                            Mail::send('emails.campaign_full_rejection_advertiser_alert',
                                ...
                                function ($mail) use ($date, $lr_build, $report_recipients, $rejc) { //Used inside function only
