...
class Lead extends Model
{
    ...
    public function scopePageOptInRateReport($query,$fullBreakdown,$params)
    {
        ...
        if($params['group_by'] == 'day') $query->groupBy('date');
    ...
    public function scopeFilterByRejection($query, $type) {
        ...
        $query->join('lead_sent_results', function($q) use($type, $duplicate_keywords, $prepop_keywords, $filter_keywords, $accepted_keywords, $not_others){
        ...
        if($type == 'acceptable') {
            $query->where(function($q) use($accepted_keywords) {
                ...
                foreach($accepted_keywords as $key) {
                    if($c == 0) $q->where('lead_sent_results.value', 'LIKE', "%$key%");
                    else $q->orWhere('lead_sent_results.value', 'LIKE', "%$key%");
                    $c++;
                }
            });
