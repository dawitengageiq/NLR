...
class UserActionLogger
{
    ...
    public function logger($section, $sub_section, $reference_id, $old_value, $new_value, $user_summary = null, $callback = null) {
        ...
        if(is_callable($callback)) {
            list($old_value, $new_value) = $callback($old_value, $new_value);
        }

        $reference = $old_value;
        if($reference == null) $reference = $new_value;
        ...
        if($sub_section != null) $section_name = isset($this->section_types[$sub_section]) ? $this->section_types[$sub_section] : $section_name;
        ...
        if($old_value == null || $new_value == null) {
            ...
            if($old_value == null && $new_value != null) { //Add
                $summary = 'Add ';
            }else if($old_value != null && $new_value == null) { //Delete
                $summary = 'Delete ';
                $severity_id = 3;
            }

            if($user_summary != null && $user_summary != '') $summary = $user_summary;
            else $summary .= $section_name.'.';
            ...
            foreach($old_value as $name => $value) {
                if($value != $new_value[$name]) {
                    ...
                    if($user_summary != null && $user_summary != '') $update_summary = $user_summary.'. Column: '.$name.'.';
                    else $update_summary = $summary.$name.'.';
    ...
    public function log($section, $sub_section, $reference_id, $user_summary = null, $old_value, $new_value, $key_mask = null, $value_mask = null) {
