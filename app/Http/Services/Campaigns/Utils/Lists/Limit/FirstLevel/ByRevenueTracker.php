...
final class ByRevenueTracker  implements \App\Http\Services\Campaigns\Utils\Lists\Contracts\LimitContract
{
    ...
    {
        if(array_key_exists('external', $limit)) $this->externalLimit = $limit['external'];
        if(array_key_exists('link_out', $limit)) $this->linkOutLimit = $limit['link_out'];
        if(array_key_exists('exit', $limit)) $this->exitLimit = $limit['exit'];
        if(array_key_exists('coreg', $limit)) $this->coregLimit = $limit['coreg'];
    }
    ...
    {
        if($this->externalLimit == null) return false;
    ...
    {
        if($this->linkOutLimit == null) return false;
    ...
    {
        if($this->exitLimit == null) return false;
    ...
    {
        if($this->coregLimit == null) return false;
