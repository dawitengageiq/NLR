...
class ByMixCoregPages extends ByPerCampaignType implements \App\Http\Services\Campaigns\Utils\Lists\Contracts\StackContract
{
    ...
    {
        list($limit, $revenueTrackerLimit, $revenueTrackerID, $campaignTypeOrder) = $param;
    ...
    protected function stackByMixCoregOrder ($campaignID)
    {
        if($this->mixCoregOrder->campaignIdExists($campaignID)) {
            // Pre populate
            if(!array_key_exists($this->mixCoregOrder->tempIndex(), $this->stack)) $this->stack[$this->mixCoregOrder->tempIndex()][0] = [];
            //get last array
