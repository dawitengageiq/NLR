...
final class CampaignList
{
    ...
    public function setParams($params)
    {
        ...
        collect($params)->each(function($param, $idx) {
            if(method_exists($this, camelCase($idx))) $this->{camelCase($idx)}($param);
        });
    ...
    public function get()
    {
        if($this->query !=  null) return $this->query->get();
        return '';
    }
     ...
     *
     * @param integer|array $campaignTypeOrder
     * @var sql $query
     * @return object class
     */
    protected function inCampaignType ($campaignTypeOrder = '')
    {
        if(!$campaignTypeOrder) return $this;
     ...
     *
     * @param integer|array excludedCampaignIds
     * @var sql $query
     * @return object class
     */
    protected function excludeCampaignIDs ($excludedCampaignIds = '')
    {
        if(!$excludedCampaignIds) return $this;
    ...
    {
        if(!$revenueTrackerID) return $this;
