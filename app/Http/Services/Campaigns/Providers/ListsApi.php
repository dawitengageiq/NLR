...
class ListsApi
{
    ...
    */
        ...
        )
    {
        ...
        if(!is_numeric($this->app->request->get('affiliate_id'))) throw new CampaignListsResolverException('invalid_affiliate_id');
        // Check for affiliate id for it is required
        if(!$this->app->request->has('affiliate_id')) throw new CampaignListsResolverException('provide_affiliate_id');
        // Check for website id for it is required
        if(!$this->app->request->has('website_id')) throw new CampaignListsResolverException('provide_website_id');
        ...
        if($this->apiConfig->isMultiPage()) $this->page = 'multi_page';
