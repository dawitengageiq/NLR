...
class Excel extends \App\Http\Services\Factories\ExportExcel
{
	...
	protected function createColumnsRange ($count, $counter = 0, $firstLetters = '')
	{
        ...
        foreach (range('A', 'Z') as $letter) {
            ...
            if ($counter == $count) return $this->columnsRange;
        }
    ...
    public function generatePerSubIDSheet($sheet)
    {
            ...
            array(
                ...
                'borders' => array(
                    'bottom' => array(
                        ...
                        'color' => array('argb' => 'A19C9C'),
                    ),
                ),
                'fill' => array(
                    'type' => \PHPExcel_Style_Fill::FILL_SOLID,
                    'color' => array('rgb' => 'CCF2EC')
                ),
                'alignment' => array(
        ...
        if($this->applyLegendsColor) $this->applylegendColors($this->headerRow);
        ...
        if($this->footer) {
            ...
            $sheet
                ...
                if($this->applyLegendsColor) $this->applylegendColors(($recordCount + ($this->headerRow + 1)));
    ...
    public function genCode($tracker) {
        ...
        if(isset($this->filters['sib_s1'])) $code .= "$tracker[s1]:";
        if(isset($this->filters['sib_s2'])) $code .= "$tracker[s2]:";
        if(isset($this->filters['sib_s3'])) $code .= "$tracker[s3]:";
        if(isset($this->filters['sib_s4'])) $code .= "$tracker[s4]";
