...
class ConsolidatedGraphController extends Controller
{
     ...
     *
     * @param Services\Consolidated\Utils\Affiliates $affiliates
     * @param Services\Contracts\ConsolidatedGraphContract $consolidatedGraph
     */
	...
	public function view (Request $request)
	{
		...
		// Process information
		if(!$this->viewData['inputs']['message']) $this->viewData['inputs']['message'] = $this->graph->message();
		...
		)
	{
		...
		// Legends ...
		list($columns, $legendsWithDesc) = $this->setExcelHeadersAndWithDescription($request->get('chart_type'));
	...
	protected function generateChartData (Request $request)
	{
		if(count($request->all())) {
			// Dummy chart
			if($this->dummyChart($request)) return;
	...
	{
		if($chartType == 'export_excel_date_range_multiple') return true;
	...
	protected function exportlink (Request $request)
	{
		...
		}else $revenueTrackerID = 'revenue_tracker_id='.$request->get('revenue_tracker_id');

		$legends = '';
		if(is_array($request->get('legends'))) $legends = http_build_query(['legends' => $request->get('legends')]);
		...
		if($request->has('date_to')) $exportLink .= '&amp;date_to=' . $request->get('date_to');
		if($legends) $exportLink .= '&amp; ' . $legends;
		...
		if($this->viewData['inputs']['sib_s1'] && $this->viewData['inputs']['sib_s1'] == 1) $exportLink .= '&amp;sib_s1=1';
		if($this->viewData['inputs']['sib_s2'] && $this->viewData['inputs']['sib_s2'] == 1) $exportLink .= '&amp;sib_s2=1';
		if($this->viewData['inputs']['sib_s3'] && $this->viewData['inputs']['sib_s3'] == 1) $exportLink .= '&amp;sib_s3=1';
		if($this->viewData['inputs']['sib_s4'] && $this->viewData['inputs']['sib_s4'] == 1) $exportLink .= '&amp;sib_s4=1';
	...
	protected function redirectData ($queryString)
	{
		...
		if(array_key_exists('chart_type', $array)) $array['chart_type'] = str_replace('export_excel_', '#', $array['chart_type']);
