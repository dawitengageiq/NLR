...
class CoregReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach($reports as $report)
        {
            ...
            if(($report->olr_only === 0 || $report->olr_only === 1) && $report->revenue_tracker_id === null) $revCol .= 'OLR:<br>';
            if($report->revenue_tracker_id != '') $revCol .= $report->revenue_tracker_id.'<br>';
    ...
    public function excel()
    {
        ...
        Excel::create($title, function($excel) use($title,$reports,$inputs,$date, $affiliates, $campaigns){ 
            $excel->sheet(Carbon::parse($inputs['lead_date'])->toDateString(),function($sheet) use ($reports,$inputs,$date, $affiliates, $campaigns){
    ...
    public function download()
    {
        ...
        if(!file_exists($file_path)) $this->excel();
