...
class CampaignList2Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $campaigns = Campaign::where('status','!=',0)->where('status','!=',3)
            ...
            ->with(['creatives' => function ($query) use($revenue_tracker_id) {
        ...
        foreach($campaigns as $campaign) {
            ...
            if(count($campaign->filter_groups) > 0) {
                ...
                if(count($filterResults) == 0 || in_array(1,$filterResults)) $filterPassed = true;
            ...
            if($limitChecker == true) {
                ...
                if($creative_id != null) $campaign_creatives[$campaign->id] = $creative_id; //Save creative id of campaign
    ...
    public function capaignListJson(Request $request)
    {
        ...
        if(isset($params['affiliate_website_check'])) {
            if((!isset($params['website_id']) || $params['website_id'] == '') && isset($params['campaign_id']) && isset($params['offer_id']) && $params['campaign_id'] != '' && $params['offer_id'] != '') {
                //Continue
            }else if(!isset($params['website_id']) || !isset($params['affiliate_id'])) {
                $response = [
            ...
            if($list) {
                ...
                if(count($new_order) > 0) {
                    ...
                    foreach($campaigns as $c) {
                        if(!in_array($c->id, $new_order)) $new_order[] = $c->id;
                        $kampaigns[$c->id] = $c;
                    }
        ...
        }else if($revenue_tracker->order_type == 1) {
        ...
        foreach($campaigns as $campaign) {
                    ...
                    if(count($filter_group->filters) > 0) {
                        ...
                        foreach($filter_group->filters as $filter) {
                            ...
                            if($filter_type == 'custom' && $filter_name == 'show_date') {
                                ...
                                if($params['screen_view'] == 2) $mobile = true;
                                else $mobile = false;
                                if($this->findCampaignFilterValue($filter, $mobile) == false) {
                                    $filterGroupError++;
                                    break;
                                ...
                                if($params['screen_view'] == 1) $desktop = true;
                                else $desktop = false;
                                if($this->findCampaignFilterValue($filter, $desktop) == false) {
                                    $filterGroupError++;
                                    break;
                                ...
                                if($params['screen_view'] == 3) $tablet = true;
                                else $tablet = false;
                                if($this->findCampaignFilterValue($filter, $tablet) == false) {
                                    $filterGroupError++;
                                    break;
                            ...
                            }else if($request->dobyear != '' && $request->dobmonth != '' && $request->dobday != '' && $filter_type == 'profile' && $filter_name == 'age') {
                            ...
                            }else if($request->ethnicity != '' && $filter_type == 'profile' && $filter_name == 'ethnicity') {
                            ...
                            }else if($request->gender != '' && $filter_type == 'profile' && $filter_name == 'gender') {
                                //Male - M
                                //Female - F
                            ...
                            }else if($filter->filter_type->type == 'profile' && $filter_name == 'zip'){
                                if($this->findCampaignFilterValue($filter, $params['zip']) == false) {
                                    $filterGroupError++;
                                    break;
                                }
                            }else if($filter->filter_type->type == 'profile' && $filter_name == 'state'){
                                if($this->findCampaignFilterValue($filter, $user->state) == false) {
                                    $filterGroupError++;
                                    break;
                                }
                            }else if($filter->filter_type->type == 'profile' && $filter_name == 'city'){
            ...
            if($limitChecker == true) {
                ...
                if($mergeCoregs && in_array($campaign_type, $mixedCoregTypes)) { //MERGE ALL MIXED COREGS
                    $campaign_type = 1;
                    if(!isset($stack[$campaign_type][0])) $stack[$campaign_type][0] = [];
                    array_push($stack[$campaign_type][0],$campaign->id);
                }
                else if(array_key_exists($campaign_type,$stack) && $campaign_type != 4 && $campaign_type != 3 && $campaign_type != 7){
                    //get last array
                    $last_set = count($stack[$campaign_type]) - 1;
                    if(count($stack[$campaign_type][$last_set]) == $stack_num_campaign) {
                        $stack[$campaign_type][count($stack[$campaign_type])][0] = $campaign->id;
        ...
        if($mergeCoregs) { //merge mixed coregs
                ...
                if(isset($stack[$value])) {
                    foreach($stack[$value] as $path){
                        if(in_array($value, $mixedCoregTypes)) {
                            if(!isset($mcLists[1][0])) $mcLists[1][0] = [];
            ...
            $has_campaign_type = [];
            foreach($stack_type_order as $value) {
                if(in_array($value, [2,8,13])) continue; //skip cuz its already merged in mixed coreg 1;
