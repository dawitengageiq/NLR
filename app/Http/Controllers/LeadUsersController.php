...
class LeadUsersController extends Controller
{
    ...
    public function allInboxEmailFeed(Request $request, $status = 0) {
        if($status != 0 && $status != 2) return 'status should only be either 0 or 2';
        ...
        foreach($settings as $setting) {
            ...
            }else if($setting->code == 'address_feed_affiliates') {
                $addFeedType = $setting->integer_value;
            }else if($setting->code == 'cvd_feed_affiliates') {
                $cvdFeedType = $setting->integer_value;
            }else if($setting->code == 'email_media_buy_affiliates') {
                $embFeedType = $setting->integer_value;
            }else if($setting->code == 'all_inbox_feed_affiliates') {
                $aibFeedType = $setting->integer_value;
            }else if($setting->code == 'epic_demand_all_inbox_feed_affiliates') {
                $edaibFeedType = $setting->integer_value;
            }else if($setting->code == 'data_feed_excluded_affiliates') {
                $exAffList = [];
        ...
        if($allowLog) Log::info('ALL INBOX CRON Status: '.$status);
        foreach($users as $user) {
            ...
            if($user->source_url != '') {
                ...
                if(count($host_names) > 2) $source_url = $host_names[count($host_names)-2] . "." . $host_names[count($host_names)-1];
                else $source_url = $parse['host'];
            }
            ...
            if($allowLog) $time1 = microtime(true);
            ...
            if($allInboxFeedStatus == 'true') {
                ...
                if($canSendAIB) {
                    ...
                    if(strpos($source_url, 'paidforresearch') !== false) 
                        $source_id = 620;
                    else if(strpos($source_url, 'epicdemand') !== false)
                        $source_id = 1232;
                    else if(strpos($source_url, 'admiredopinions') !== false)
                        $source_id = 1104;
                    else if(strpos($source_url, 'clinicaltrialshelp') !== false)
                        $source_id = 620;
                    else $source_id = 620;
                    ...
                    $all_inbox_curl->post('https://api3.all-inbox.com/ClientDataAPI-V2/?resource=contacts',array(
            ...
            if($embFeedStatus == 'true') {
                ...
                if($canSendEMB) {
                    $emb_curl = new Curl();
                    $emb_curl->post('https://api3.all-inbox.com/ClientDataAPI-V2/?resource=contacts',array(
                        "api_key" => "d41fe959_8cfb_48de_98db_e86bb601820e",
                        "source_id" => 985,
                        "email" => $user->email,
                        "first_name" => $user->first_name,
