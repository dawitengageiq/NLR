...
class FilterController extends Controller
{
        ...
        foreach($campaigns as $campaign) {
            ...
            if($uniCapPassed && $affCapPassed) {
                ...
                if($filter_process_status == 1) {
                    ...
                    $filter_groups_pass_checker = array();
                    ...
                    if(count($filter_groups) > 0) { //check if campaign has filter groups
                        foreach($filter_groups as $group) { //get all campaign filter groups
                            if( count($group->filters) > 0 && $group->status == 1) { //check if filter group has filters and is active
                                ...
                                foreach($group->filters as $filter) { // go through each filter
                                    ...
                                    else if($filter->filter_type->type == 'profile' && $filter->filter_type->name == 'ethnicity')
                                    {
                                    ...
                                    else if($filter->filter_type->type == 'profile' && $filter->filter_type->name == 'gender')
                                    {
                                        //Male - M
                                        //Female - F
                                    ...
                                    else if($filter->filter_type->type == 'profile' && $filter->filter_type->name == 'email')
                                    {
                                    ...
                                    else if($filter->filter_type->type == 'profile' && array_key_exists($filter->filter_type->name, $user) )
                                    {
                                    ...
                                    else if($filter->filter_type->type == 'question') {
                                    ...
                                    else if($filter->filter_type->type == 'custom' && $filter->filter_type->name == 'show_date') {
                                    ...
                                    else if($filter->filter_type->type == 'custom' && $filter->filter_type->name == 'show_time') {
                                    ...
                                    else if($filter->filter_type->type == 'custom' && $filter->filter_type->name == 'mobile_view') {
                                        if($user['screen_view'] == 2) $mobile = true;
                                        else $mobile = false;
                                    ...
                                    else if($filter->filter_type->type == 'custom' && $filter->filter_type->name == 'desktop_view') {
                                        if($user['screen_view'] == 1) $desktop = true;
                                        else $desktop = false;
                                    ...
                                    else if($filter->filter_type->type == 'custom' && $filter->filter_type->name == 'tablet_view') {
                                        if($user['screen_view'] == 3) $tablet = true;
                                        else $tablet = false;
                                    ...
                                    else if($filter->filter_type->type == 'custom' && $filter->filter_type->name == 'check_ping') {
                        ...
                        if( count($filter_groups_pass_checker) == 0 || in_array(1,$filter_groups_pass_checker)) $filterError = 0;
                        else $filterError = 1;
                    }else {
                        $filterError = 0;
                    }
                    ...
                    if($campaign->campaign_type == 5 && $filterError == 0){
                        if(in_array($campaign->linkout_offer_id,$cake_clicks)) $filterError = 1;
                    }
                }
                if($filterError == 0) {
                    ...
                    if($campaign->campaign_type == 4) { //External Path
                        ...
                        } else if($external_counter < $external_limit) {
                            $passedCampaigns[] = $campaign->id;
                            $external_counter++;
                        }
                    }else if($campaign->campaign_type == 5) { //Link Out
                        if($linkout_limit == null) {
                            $passedCampaigns[] = $campaign->id;
                        } else if($linkout_counter < $linkout_limit) {
                            $passedCampaigns[] = $campaign->id;
                            $linkout_counter++;
                        }
                    }else if($campaign->campaign_type == 6){ //Exit Page
                        ...
                        if($coreg_limit == null) { //Co-reg
                            $passedCampaigns[] = $campaign->id;
                        } else if($coreg_counter < $coreg_limit) {
                            $passedCampaigns[] = $campaign->id;
                            $coreg_counter++;
                        }
                    }
    ...
    private function findCampaignFilterValue($filter, $answer)
    {
        ...
        if(! is_null($filter->value_text) || $filter->value_text != '')
        {
            ...
            if(strpos($filter->value_text,'[NOT]') !== FALSE) {
                ...
                if($answer != $filter_value) $value = true;
            } else if($answer == strtolower($filter->value_text)) $value = true;
        }
        else if(! is_null($filter->value_boolean) || $filter->value_boolean != '')
        {
            if($answer == $filter->value_boolean) $value = true;
        }
        else if(! is_null($filter->value_min_date) || $filter->value_min_date != '' || ! is_null($filter->value_max_date) || $filter->value_max_date != '')
        {
            $min = Carbon::parse($filter->value_min_date);
            $max = Carbon::parse($filter->value_min_date);
            $value = Carbon::parse($answer)->between($min, $max);
        }
        else if(! is_null($filter->value_min_time) || $filter->value_min_time != '' || ! is_null($filter->value_max_time) || $filter->value_max_time != '')
        {
            $min = Carbon::parse($filter->value_min_time);
            $max = Carbon::parse($filter->value_max_time);
            $value = Carbon::parse($answer)->between($min, $max);
        }
        else if(! is_null($filter->value_min_integer) || $filter->value_min_integer != '' || ! is_null($filter->value_max_integer) || $filter->value_max_integer != '')
        {
            if($answer >= $filter->value_min_integer && $answer <= $filter->value_max_integer) $value = true;
        }
        else if(! is_null($filter->value_array) || $filter->value_array != '')
        {
            $answer = strtolower($answer);
            if(strpos($filter->value_array,'[NOT]') !== FALSE) {
                ...
                if(!in_array($answer,$array)) $value = true;
            } else {
    ...
    public function checkPing($filter,$user)
    {
        ...
        if(strpos($url,'[VALUE_EMAIL]') !== false) $url = str_replace('[VALUE_EMAIL]', $user['email'], $url);
    ...
    public function returnCampaignById($id)
    {
        ...
        if($campaign->content) $content = $campaign->content->long;
        else $content = 'NONE';
            ...
            if($uniCapPassed && $affCapPassed) {
                        ...
                        foreach($filter_groups as $group) { //get all campaign filter groups
                            if( count($group->filters) > 0 && $group->status == 1) { //check if filter group has filters and is active
                                ...
                                foreach($group->filters as $filter) { // go through each filter
                                    ...
                                    else if($filter_type == 'profile' && $filter_name == 'ethnicity')
                                    {
                                        $ethnicity = array_search($user['ethnicity'], $ethnic_groups);
                                        // $ethnicity = $ethnic_groups[$user['ethnicity']];
                                        if($user['ethnicity'] != 0) {
                                    ...
                                    else if($filter_type == 'profile' && $filter_name == 'gender')
                                    {
                                        //Male - M
                                        //Female - F
                                        $gender = $user['gender'] == 'M' ? 'Male' : 'Female';
                                    ...
                                    else if($filter_type == 'profile' && $filter_name == 'email')
                                    {
                                        $email_split = explode('@',$user['email']);
                                        $domain = explode('.',$email_split[1]);
                                    ...
                                    else if($filter_type == 'profile' && array_key_exists($filter_name, $user) )
                                    {
                                    ...
                                    else if($filter_type == 'custom' && $filter->filter_type->name == 'show_date') {
                                        $date_today = Carbon::now()->format('l');
                                        // Log::info($date_today);
                                        if($this->findCampaignFilterValue($filter, $date_today) == false) {
                                            $filterGroupError++;
                                    ...
                                    else if($filter_type == 'custom' && $filter_name == 'show_time') {
                                        $time_today = Carbon::now()->format('H:i');
                                        if($this->findCampaignFilterValue($filter, $time_today) == false) {
                                            $filterGroupError++;
                                            break;
                                    ...
                                    else if($filter_type == 'custom' && $filter_name == 'mobile_view') {
                                        if($user['screen_view'] == 2) $mobile = true;
                                        else $mobile = false;
                                        if($this->findCampaignFilterValue($filter, $mobile) == false) {
                                            $filterGroupError++;
                                    ...
                                    else if($filter_type == 'custom' && $filter_name == 'desktop_view') {
                                        if($user['screen_view'] == 1) $desktop = true;
                                        else $desktop = false;
                                        if($this->findCampaignFilterValue($filter, $desktop) == false) {
                                            $filterGroupError++;
                                    ...
                                    else if($filter_type == 'custom' && $filter_name == 'tablet_view') {
                                        if($user['screen_view'] == 3) $tablet = true;
                                        else $tablet = false;
                                        if($this->findCampaignFilterValue($filter, $tablet) == false) {
                                            $filterGroupError++;
                                    ...
                                    else if($filter_type == 'custom' && $filter_name == 'check_ping') {
                                        $result = $this->checkPing($filter,$user);
                                        if($this->findCampaignFilterValue($filter, $result) == false) {
                                            $filterGroupError++;
                                            break;
                ...
                if($filterError == 0) {
                    ...
                    if($limitChecker == true) {
                        ...
                        }else if($campaign_type == 4 || $campaign_type == 3 || $campaign_type == 7) {
                            //1 Display per Page: External, Long Form (1st Grp) & (2nd Grp)
                            if(isset($stack[$campaign_type])) $key = count($stack[$campaign_type]);
                            else $key = 0;
    ...
    public function campaignStackProcessorPhp(Request $request)
    {
        ...
        if(empty($campaigns) || is_null($campaigns)) return ['message' => 'Campaign does not exists.'];
        ...
        if($campaign_type == null) return ['message' => 'Campaign does not exists.'];
        ...
        if($rtSetting && $rtSetting->subid_breakdown == 0) { //Check if rev tracker allowed for subid breakdown
            ...
            if($rtSetting && $rtSetting->sib_s1 == 0) $s1 = '';
            if($rtSetting && $rtSetting->sib_s2 == 0) $s2 = '';
            if($rtSetting && $rtSetting->sib_s3 == 0) $s3 = '';
            if($rtSetting && $rtSetting->sib_s4 == 0) $s4 = '';
            $s5 = '';
        }
        ...
        if(env('EXECUTE_REORDER_MIXED_COREG_CAMPAIGNS', false) || env('EXECUTE_DAILY_REORDER_MIXED_COREG_CAMPAIGNS', false)) {
            if(isset($rs['mixed_coreg_campaign_reordering_status']) && $rs['mixed_coreg_campaign_reordering_status'] == 1) $mcReorder = true;
        }
        //Check Campaign Type Reorder Global Settings
        $ctReorder = false;
        if(env('EXECUTE_REORDER_CAMPAIGNS', false)) {
            if(isset($rs['campaign_reordering_status']) && $rs['campaign_reordering_status'] == 1) $ctReorder = true;
        }
        //Check Rev Tracker Settings Level
        if($mcReorder || $ctReorder) {
            if($rtSetting) {
                if($rtSetting->mixed_coreg_order_status != 1) $mcReorder = false;
                if($rtSetting->order_status != 1) $ctReorder = false;
            }
        }
        ...
        foreach($campaigns as $id)
        {
            ...
            if(in_array($campaign_type,$campaigns_types_that_stacked) && $stack_content != '') $return_str .= '<div class="separator" style=""></div>';
        ...
        if(in_array($campaign_type,$campaigns_types_that_stacked)) $return_str .= '<div align="center"><button id="submit_stack_button" type="button" class="submit_button_form" align="absmiddle">Submit</button></div>';
    ...
    */
    public function getCampaignCreativeContent($content,$creative_id,$creative) {
        if($creative) {
            ...
            if(strpos($html,'[VALUE_CREATIVE_IMAGE]') !== false) $html = str_replace('[VALUE_CREATIVE_IMAGE]', $creative['image'] , $html);
    ...
    */
    ...
    public function zipValidationChecker(Request $request)
    {
        ...
        if(is_null($details)) $checker = 'Please provide a valid US Zip Code';
        else $checker = 'true';
    ...
    public function registerUserAndGetCampaigns(Request $request)
    {
        ...
        if($revenue_tracker) $path_type = $revenue_tracker->path_type;
        else {
            $default_rev_tracker = AffiliateRevenueTracker::select('path_type')->where('revenue_tracker_id',1)->first();
        ...
        foreach($qualified_campaigns as $page) {
            foreach($page as $cid) {
                ...
                if($creative_id != null) $creatives[$cid] = $creative_id; //Save creative id of campaign
            }
        }
    ...
    public function getQualifiedCampaigns($user,$revenue_tracker,$path_type)
    {
        ...
        /*** For Testing ***/$lead_duplicate_total = 0;$lead_cap_total = 0;$campaign_filter_total = 0;$cake_checker_total = 0;$final = 0;
        ...
        foreach($campaigns as $campaign) {
            ...
            /* CHECK IF USER ALREADY ANSWERED CAMPAIGN */
            if(in_array($campaign->id,$lead_campaigns)) continue; //If already answered, skip campaign
            ...
            if($uniCapPassed == false) continue; //If default campaign cap failed, skip campaign
            ...
            if($affCapPassed == false) continue; //If affiliate campaign cap failed, skip campaign
            ...
            if($uniCapPassed && $affCapPassed) {
                if($filter_process_status == 1) { // Check campaign filter process' status; If Active(1) proceed with filter, else disregard filters.
                    ...
                    if(count($filter_groups) > 0) { //check if campaign has filter groups
                        foreach($filter_groups as $group) { //go through each filter group
                            ...
                            if($group->status == 1) { //check if filter group is active
                                ...
                                if($filters) { //check if filter group has filters
                                    ...
                                    if($filterGroupError == 0) $campaign_filter_passed = true;
                                }
                            }
                            $filter_groups_pass_checker[] = $filterGroupError == 0 ? 1 : 0;
                        ...
                        if($campaign_filter_passed || count($filter_groups_pass_checker) == 0 || in_array(1,$filter_groups_pass_checker)) $filterError = 0;
                        else $filterError = 1;
                    }else {
    ...
    private function ifFilterPassed($filter,$filter_types,$user)
    {
        ...
        if($filter_type == 'profile') {
            switch($filter_name) {
                ...
                case 'ethnicity':
                    $ethnic_groups = config('constants.ETHNIC_GROUPS');
                    $ethnicity = array_search($user['ethnicity'], $ethnic_groups);
                    if($user['ethnicity'] != 0) $answer = $ethnicity; // if ethnicity is not 0, if 0; answer = ''
                    break;
                case 'gender':
                    //Male - M
                    //Female - F
        ...
        if($this->findCampaignFilterValue($filter, $answer) == false) $return_value = false;
    ...
    public function getCampaignContentByID(Request $request)
    {
        ...
        /* For Creatives */
        $creatives = [];
        if($request->has('creatives')) $creatives = $request->input('creatives');
    ...
    public function getPathAddDetails(Request $request) {
        ...
        if(stripos($path_url, 'admiredopinion') !== false) {
            $tcpa = 'admired_tcpa';
        }else if(stripos($path_url, 'paidforresearch') !== false) {
            $tcpa = 'pfr_tcpa';
        }else if(stripos($path_url, 'clinicaltrialshelp') !== false) {
        ...
        if(isset($inputs['get_filter_free_campaigns']) && $inputs['get_filter_free_campaigns'] == 1) {
            ...
            foreach($campaigns as $campaign) {
                ...
                if($capCheck) {
                    ...
                    if($creative_id != null) 
                        $creatives[$campaign->id] = $creative_id; //Save creative id of campaign
                }
